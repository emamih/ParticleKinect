...
void demoParticle::update(){
	...
	if( mode == PARTICLE_MODE_ATTRACT ){
		//if(loopCnt < 5){
			...
			ofPoint attractPt(jointPos.x, jointPos.y);
			//ofLog() << "x: " << possX;
			//ofLog() << "y: " << possY;
...
void demoParticle::draw(){
	...
	float tmpColor = ofMap((pos.distance(jointPos)),0,ofGetWidth(),0,255,true);
	if( mode == PARTICLE_MODE_ATTRACT ){
		ofSetColor(tmpColor,255-tmpColor, 180);
	}
	else if( mode == PARTICLE_MODE_REPEL ){
		ofSetColor(tmpColor, 255-tmpColor, 63);
	}
	else if( mode == PARTICLE_MODE_NOISE ){
